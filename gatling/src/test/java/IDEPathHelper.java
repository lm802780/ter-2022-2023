...
public class IDEPathHelper {
    ...
    static {
        try {
            Path projectRootDir = Paths.get(requireNonNull(IDEPathHelper.class.getResource("gatling.conf"), "Couldn't" +
                    " locate gatling.conf").toURI()).getParent().getParent().getParent();
